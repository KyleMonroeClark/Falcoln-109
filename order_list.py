...
class order_list_Frame(tk.Frame):
    ...
    def __init__(self, parent, controller, csv_file="orders.csv"):
        ...
        # Get the correct file path for the CSV
    ...
    def create_buttons(self):
        ...
        tk.Button(button_frame, text="Mark Ordered", command=self.mark_ordered, bg="green", fg="white").pack(side=tk.LEFT, padx=5)
    ...
    def mark_ordered(self):
        ...
        for order in self.orders:
            if order["Order Number"] == order_number:
                order["Is Ordered"] = "True"
                order["Marked Ordered Date"] = date.today().strftime("%Y-%m-%d")
                break
